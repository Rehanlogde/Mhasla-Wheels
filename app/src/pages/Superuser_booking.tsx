...
// AdminBookings logic reused for superuser actions
var bookingstateupdate = "select action"
var vehicleidofbooking = ""

export default function Superuser_booking () {
...
const [message, setMessage] = useState<string | null>(null);
    ...
    const getsuperuserdata = async()=>{
        ...
        const result = await fetch("/api/functions/getsuperuserdata")
    ...
    // AdminBookings confirm/reject/status logic
    ...
    const getbookingdetails = async () => {
        console.log("getting the booking details")
        const result = await fetch("/api/functions/superuser_booking_details")
        const finalresponse = await result.json()
        if (finalresponse.ok) {
            console.log("got the result !")
            console.log(finalresponse.data)
            setbookingsdata(finalresponse.data)
        } 
        else{
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-3">
                <h2 className="text-2xl font-bold text-red-500">Bookings Overview</h2>
                <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                    ...
                    <Button onClick={getbookingdetails} variant="primary">
                        Refresh
                    </Button>
                </div>
            </div>
      ...
      <p className={`text-2xl font-bold ${stat.color}`}>{stat.count}</p>
                    ...
                    <tbody>
                        {pageData.map((b) => {
                            ...
                            return (
                                <tr
                                    key={b.booking_code}
                                    className="hover:bg-[#1E1E1E] transition cursor-pointer"
                                ...
                                >
                                    ...
                                    <td className="p-3 border-b border-gray-800">
                                        {b.drivername || "-"} <br />
                                        {superuserdata['adminaccess'] && (
                                            <button onClick={e => {e.stopPropagation(); setChangingDriverFor(b)}}>Change the driver</button>
                                        )}
                                    </td>
                                    ...
                                    <td className="p-3 border-b border-gray-800 flex gap-2 flex-wrap">
                                        ...
                                        {superuserdata['adminaccess'] && b.status == "pending" && (
                                            <div>
                                                <Button
                                                    size="sm"
                                                    ...
                                                    onClick={e => {
                                                        ...
                                                        if (b.drivername != null) {
                                                            ...
                                                            toast.error("Please first select driver before proceeding.")
                                                        }
                                                    }}
                                                >
                                                    Confirm
                                                ...
                                                <Button
                                                    size="sm"
                                                    variant="destructive"
                                                    onClick={e => {e.stopPropagation(); rejectBooking(b.booking_code)}}
                                                >
                                                    Reject
                                                </Button>
                                            </div>
                                        )}
                                        {superuserdata['adminaccess'] && b.status != "rejected" && b.status != "pending" && (
                                            <div>
                    ...
                    <div className="bg-[#111] border border-gray-800 rounded-xl shadow-xl p-6 max-w-lg w-full text-gray-200">
                        ...
                        <div className="space-y-2 text-sm text-gray-300">
                            ...
                            {selected.custom_journey_details && (
                                <p><strong>Custom Details:</strong> {selected.custom_journey_details}</p>
                            )}
                            {selected.custom_rate && (
                                <p><strong>Custom Rate:</strong> ₹{selected.custom_rate}</p>
                            )}
                            {selected.custom_unit && (
                                <p><strong>Unit:</strong> {selected.custom_unit}</p>
                            )}
                            {selected.depart_date && (
                                <p><strong>Depart:</strong> {new Date(selected.depart_date).toLocaleString()}</p>
                            )}
                            {selected.return_date && (
                                <p><strong>Return:</strong> {new Date(selected.return_date).toLocaleString()}</p>
                            )}
                            {selected.ride_date && (
                                <p><strong>Ride Date:</strong> {new Date(selected.ride_date).toLocaleString()}</p>
                            )}
                            ...
                            <p><strong>Updated:</strong> {selected.updated_at ? new Date(selected.updated_at).toLocaleString() : "-"}</p>
                        </div>
            ...
            {changingDriverFor && (
                <ChangeDriverForm
                    ...
                    onDriverChanged={getbookingdetails}
                />
            )}
        </div>
    )
